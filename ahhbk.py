...
def get_hwid():
    ...
    id_file = os.path.expanduser("~/.ahb_id")
    ...
    try:
        ...
        if android_id:
            ...
            hwid = hashlib.md5(f"AHB-{brand}-{model}-{android_id}".encode()).hexdigest().upper()
        else:
        ...
        # Absolute fallback
        fallback_id = "AHB-" + str(uuid.uuid4())[:12].upper()
        try:
...
def main_menu_entry():
    while True:
        ...
        if choice == '1': return True
            ...
            msg = f"Salam Ali Khan, I want to get the approval key for AHB/MODEL 👑. My HWID is: {get_hwid()}"
...
def windows():
    ...
    A = f"Mozilla/5.0 (Windows; U; Windows NT {str(random.choice(range(5, 7)))}.1; en-US) AppleWebKit/534.{aV} (KHTML, like Gecko) Chrome/{str(random.choice(range(8, 12)))}.0.{str(random.choice(range(1, 5000)))}"
    bV = str(random.choice(range(1, 36)))
    bx = str(random.choice(range(34, 38)))
    bz = f'5{bx}.{bV}'
    B = f"Mozilla/5.0 (Windows NT {str(random.choice(range(5, 7)))}.{str(random.choice(['2', '1']))}) AppleWebKit/{bz} (KHTML, like Gecko) Chrome/{str(random.choice(range(12, 42)))}.0.{str(random.choice(range(1, 5000)))}"
    cV = str(random.choice(range(1, 36)))
    cx = str(random.choice(range(34, 38)))
    cz = f'5{cx}.{cV}'
    C = f"Mozilla/5.0 (Windows NT 6.{str(random.choice(['2', '1']))}; WOW64) AppleWebKit/{cz} (KHTML, like Gecko) Chrome/{str(random.choice(range(12, 42)))}.0.{str(random.choice(range(742, 2200)))}"
    D = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.{str(random.choice(range(1, 7120)))}.0 Safari/537.36"
    return random.choice([A, B, C, D])

def window1():
    aV = str(random.choice(range(10, 20)))
    A = f"Mozilla/5.0 (Windows; U; Windows NT {random.choice(range(6, 11))}.0; en-US) AppleWebKit/534.{aV} (KHTML, like Gecko) Chrome/{random.choice(range(80, 122))}.0.{random.choice(range(4000, 7000))}"
...
def login_1(uid):
    ...
    try:
        with console.lock:
            ...
            msg = console._apply_tags(f"\r {DIM}[{ts}]{RESET} [bold green]AHB/MODEL 👑[/bold green] [bold yellow]{loop}[/bold yellow] [bold green]OK:{len(oks)}[/bold green] [bold red]CP:{len(cps)}")
