...
def linex(): console.print(f"[dim]{'â”€' * get_terminal_width()}[/dim]")

def show_panel(text, title=None, subtitle=None, style="dim"):
    width = get_terminal_width()
    top = f"â•­â”€â”€â”¤ {title} â”œ" if title else "â•­"
    if subtitle: top += f"â”€â”¤ {subtitle} â”œ"
    top += "â”€" * (width - len(console._strip_tags(top)) - 1) + "â•®"
    console.print(f"[{style}]{top}[/{style}]")
    for line in text.strip('\n').split('\n'):
        clean = console._strip_tags(line); padding = width - len(clean) - 2
        console.print(f"[{style}]â”‚[/{style}] {line}{' ' * max(0, padding)} [{style}]â”‚[/{style}]")
    bottom = "â•°" + "â”€" * (width - 2) + "â•¯"
    console.print(f"[{style}]{bottom}[/{style}]")

def show_success(msg): console.print(f" [bold green]âœ“[/bold green] {msg}")
def show_error(msg): console.print(f" [bold red]Ã—[/bold red] {msg}")
...
def add_key():
    ...
    linex()
    console.print(" â—† [bold yellow]1[/bold yellow]  7 Days")
    console.print(" â—† [bold yellow]2[/bold yellow]  15 Days")
    console.print(" â—† [bold yellow]3[/bold yellow]  30 Days")
    console.print(" â—† [bold yellow]4[/bold yellow]  Lifetime")
    linex()
    ...
    if db.set_data(f'keys/{key}', data):
        res = f"--- key created\n â–Ž [bold green]user[/bold green]   {user_name}\n â–Ž [bold green]key[/bold green]    {key}\n â–Ž [bold green]expiry[/bold green] {expiry_date}"
...
def view_keys():
    ...
    if not keys_data:
        ...
        for k, v in keys_data.items():
            ...
            console.print(f" â—† [bold cyan]{k}[/bold cyan] | {v.get('name', 'N/A'):<10} | Exp: {v.get('expiry', 'N/A')} | {hwid_stat}")
    linex()
    safe_input("\n [?] Press Enter to continue...")
...
def manage_key():
    ...
    while True:
        admin_header()
        info = f"--- key info\n â–Ž [bold cyan]user[/bold cyan]   {key_data.get('name')}\n â–Ž [bold cyan]expiry[/bold cyan] {key_data.get('expiry')}\n â–Ž [bold cyan]hwid[/bold cyan]   {key_data.get('hwid', 'Not Registered')}"
        show_panel(info, title=f"MANAGING: {target_key}", style="yellow")
        
        console.print("\n [bold yellow]ACTIONS[/bold yellow]")
        linex()
        console.print(" â—† [bold yellow]01[/bold yellow]  Reset HWID (Unlock Device)")
        console.print(" â—† [bold yellow]02[/bold yellow]  Update Expiry Date")
        console.print(" â—† [bold yellow]03[/bold yellow]  Delete Key")
        console.print(" â—† [bold yellow]00[/bold yellow]  Back")
        linex()
...
def main():
    while True:
        ...
        linex()
        console.print(" â—† [bold yellow]01[/bold yellow]  Generate New Key")
        console.print(" â—† [bold yellow]02[/bold yellow]  View Generated Keys")
        console.print(" â—† [bold yellow]03[/bold yellow]  Manage Existing Key")
        console.print(" â—† [bold yellow]00[/bold yellow]  Exit")
        linex()
        choice = safe_input("\n [?] Selection : ")
        if choice == '1': add_key()
